   # Sprint 1:

   ## `1. AI hunting for LinkedIn!`
1. Tripple benefit. Icons at the end.
 
   ![Alt text](AI_hunting_on_LinkedIn/1Linked.png)
    
2. Long dash sign, icons instead of bulletpoints.
   
    ![Alt text](AI_hunting_on_LinkedIn/2Linked.png)

3. Concerned about this one.
   
    ![Alt text](AI_hunting_on_LinkedIn/3Linked.png)
4. Long dash, consequent tripples.

    ![Alt text](AI_hunting_on_LinkedIn/4Linked.png)
5. Long dash, tripple bullet. Ai like icon sused.
   
    ![Alt text](AI_hunting_on_LinkedIn/5Linked.png)
6. Dont know...
   
    ![Alt text](AI_hunting_on_LinkedIn/6Linked.png)
7. Many icons, tripples (five-pples?)

    ![Alt text](AI_hunting_on_LinkedIn/7Linked.png)
8. Long dash, icons.
   
    ![Alt text](AI_hunting_on_LinkedIn/8Linked.png)
9.  Icons looks non human like.
    
    ![Alt text](AI_hunting_on_LinkedIn/9Linked.png)
10. Text structure, bolding. Text goes longer, then non human like icons starts to popup.
    
    ![Alt text](AI_hunting_on_LinkedIn/10Linked.png)
